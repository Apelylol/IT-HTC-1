<!-- workshop-header -->
<img width="1347" height="127" alt="Coding Thailand 2026 header" src="https://github.com/user-attachments/assets/ba5cf267-f460-4fb0-b69b-c461ae061a3b" />

# 📝 Worksheet W3 — Prediction Log

> **ทำในช่วง 14:00-15:30**
> ⭐ **สำคัญที่สุด** — เป็นหลักฐานหลักของหัวข้อ Data & Testing (5 คะแนน)

---

## ข้อมูลทีม

- **ชื่อทีม:** _________________
- **Model Version ที่ทดสอบ:** ☐ V1 (ทดสอบครั้งแรก)  ☐ V2 (หลัง iterate)

---

## V1 — Test ครั้งแรก (อย่างน้อย 10 cases)

### Setup

- **เวลาเริ่มทดสอบ:** _____
- **เวลาเสร็จ:** _____
- **Model link ใน Edge Impulse:** _________________
- **สภาพแวดล้อมที่ทดสอบ:** _________________

### Prediction Log V1

| # | Case ที่ทดสอบ | Predicted Class | Confidence | Actual Class | ถูก/ผิด | หมายเหตุ |
|---|---|---|---|---|---|---|
| 1 | ตัวอย่าง: เดินช้าๆ ในห้อง | เดิน | 0.89 | เดิน | ✅ ถูก | ดี |
| 2 | ตัวอย่าง: นั่งลงเร็วๆ | ล้ม | 0.62 | นั่ง | ❌ ผิด | confidence ไม่สูง, classifier confuse กับการเปลี่ยน posture เร็ว |
| 3 | | | | | | |
| 4 | | | | | | |
| 5 | | | | | | |
| 6 | | | | | | |
| 7 | | | | | | |
| 8 | | | | | | |
| 9 | | | | | | |
| 10 | | | | | | |

### V1 Summary

| Metric | ค่า |
|---|---|
| Total cases | _____ |
| Correct | _____ |
| Wrong | _____ |
| **Accuracy** | _____% |
| Average confidence (ถูก) | _____ |
| Average confidence (ผิด) | _____ |

### Confusion Pattern V1

ระบุ pattern ที่ผิด:

```
1. _________________________________________________
   ตัวอย่าง: "นั่งลงเร็วๆ" ถูกจัดเป็น "ล้ม" 3 ครั้งจาก 5

2. _________________________________________________

3. _________________________________________________
```

---

## 🔄 Iteration Plan — แก้อะไรใน V2?

จาก V1 analysis จะปรับอะไร? (ทำได้หลายข้อ)

- [ ] เก็บข้อมูลเพิ่มใน class ที่ผิดบ่อย
  - **Class:** _________________
  - **จำนวน:** _____ samples
  - **Variation ใหม่:** _________________

- [ ] แก้ class definition
  - **Class:** _________________
  - **แก้เป็น:** _________________

- [ ] เพิ่ม class ใหม่
  - **Class:** _________________
  - **เหตุผล:** _________________

- [ ] ปรับ model parameter
  - **Parameter:** _________________
  - **จาก:** ___ **เป็น:** ___

- [ ] อื่นๆ: _________________

### Hypothesis

```
เราคาดว่าหลัง iterate แล้ว accuracy จะเพิ่มจาก ____% เป็น ____%
เพราะ _________________________________________________
```

---

## V2 — Test หลัง iterate

### Prediction Log V2

ใช้ test cases เดิม + เพิ่ม 5 cases ใหม่ที่ท้าทาย

| # | Case ที่ทดสอบ | Predicted | Confidence | Actual | ถูก/ผิด | เทียบกับ V1 |
|---|---|---|---|---|---|---|
| 1 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 2 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 3 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 4 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 5 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 6 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 7 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 8 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 9 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 10 | | | | | | ☐ ดีขึ้น ☐ เท่าเดิม ☐ แย่ลง |
| 11 | | | | | | new case |
| 12 | | | | | | new case |
| 13 | | | | | | new case |
| 14 | | | | | | new case |
| 15 | | | | | | new case |

### V2 Summary

| Metric | V1 | V2 | Δ |
|---|---|---|---|
| Accuracy | __% | __% | +/-__% |
| Avg confidence (ถูก) | __ | __ | +/-__ |
| Avg confidence (ผิด) | __ | __ | +/-__ |

---

## 📊 Comparison V1 vs V2

ตอบเป็นข้อความ:

### ดีขึ้นเพราะอะไร?
```
[เขียน]
```

### แย่ลงตรงไหน? (ถ้ามี)
```
[เขียน — ตอบตรงๆ ไม่ต้องกลัวถูกหักคะแนน]
```

### ถ้ามีเวลาอีก จะทำ V3 ยังไง?
```
[เขียน]
```

---

## 📤 วิธี submit

```bash
git add worksheets/W3-prediction-log.md
git commit -m "test: เพิ่ม prediction log V1 และ V2, accuracy เพิ่มจาก X% เป็น Y%"
git push
```

แนะนำให้ commit ครั้งหลัง V1 เสร็จ และอีกครั้งหลัง V2 เสร็จ (จะได้มี history ที่ดี)

---

## 💡 Tips

1. **ตอบตรงๆ ว่าโมเดลห่วยตรงไหน** — ครูดูที่ insight ไม่ใช่ accuracy เริ่ดๆ
2. **บันทึก confidence** เสมอ — บอกได้ว่าโมเดลแน่ใจหรือเดา
3. **Edge cases** สำคัญกว่า normal cases — มันคือที่ทดสอบ robustness
4. **ถ้า V2 แย่กว่า V1** — เป็นเรื่องปกติ! บอก hypothesis ว่าทำไม จะได้คะแนนเต็มเหมือนกัน
